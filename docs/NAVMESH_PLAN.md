...
Design doc for the next phase. The user named this as the next work after the Hotan Kingdom zone visual (Phase 1) landed. Goal: render silk-nav's polygon-based walkable surface + walls as a togglable overlay on top of the existing zone scene.

**Update (2026-05-14).** Phase 1.5 shipped a standalone navmesh edge viewer (`navmesh.py` + `parsers/`; see [`NAVMESH.md`](NAVMESH.md)) that derisks the parser path and the JS bridge pattern for line-segment overlays. Phase 2 now has two viable inputs (see "Open design questions" below — the first one is partly resolved by Phase 1.5).
...
1. **Where does the export run?** silk-nav's repo, or ours? Two viable paths now:
   - **(a) silk-nav-side export → vendored JSON.** Mirrors the existing zone-pipeline pattern (silk-nav owns SRO+nav; we vendor outputs). One-shot precompute; cached zone-level JSON is fast to load. Doesn't reuse the parsers we ported in Phase 1.5.
   - **(b) Use the in-repo `parsers/` directly inside `zone.py`.** Reuses Phase 1.5's parser path (no JSON round-trip). Computes overlay data at zone-load time; closer to live PK2 dump. Adds the polygon-graph + CC computation (silk-nav's `nav.merge` + `nav.zone_polygon_graph`) as a new dep — those modules aren't ported into our `parsers/` yet.
   - **Recommendation:** start with (a) for cache-friendliness and parity with Phase 1; revisit (b) only if iteration speed on the silk-nav side becomes a bottleneck.
...
- [`NAVMESH.md`](NAVMESH.md) — Phase 1.5 standalone viewer. Reuse-candidate: parser layer, EdgeFlag color palette, JS bridge pattern (`runJavaScript` fire-and-forget for overlays), tristate tree visibility.
